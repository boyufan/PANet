...
class BBoxUtility(object):
    ...
    def detection_out(self, predictions, background_label_id=0, keep_top_k=200,
        ...
        """
        mbox_loc = predictions[:, :, :4] # 4
        variances = predictions[:, :, -4:] # 4
        mbox_priorbox = predictions[:, :, -8:-4] # 4
        mbox_conf = predictions[:, :, 4:-8] # NUM_CLASSES

        results = []
        for i in range(len(mbox_loc)):  # len(mbox_loc): batch size
            results.append([])
