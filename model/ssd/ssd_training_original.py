...
class MultiboxLoss(object):
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        pos_loc_loss = tf.reduce_sum(loc_loss * y_true[:, :, -8],
                                     reduction_indices=1)
        # print("pll", pos_loc_loss, loc_loss)
        pos_conf_loss = tf.reduce_sum(conf_loss * y_true[:, :, -8],
                                      reduction_indices=1)
        # print("pcl", pos_conf_loss, conf_loss)
        ...
        has_min = tf.cast(tf.reduce_any(pos_num_neg_mask, 0), dtype=tf.float32)
        # num_neg = tf.concat(0, [num_neg,
        #                         [(1 - has_min) * self.negatives_for_hard]])
        # print(num_neg, [(1 - has_min) * self.negatives_for_hard], has_min, self.negatives_for_hard)
        ...
        # print(pos_conf_loss, neg_conf_loss, total_loss)
        total_loss /= (num_pos + tf.to_float(num_neg_batch))
        num_pos = tf.where(tf.not_equal(num_pos, 0), num_pos,
                            tf.ones_like(num_pos))
        # print(self.alpha, pos_loc_loss, num_pos, total_loss)
